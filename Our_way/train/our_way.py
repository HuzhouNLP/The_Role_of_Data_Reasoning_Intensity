...
def generate_paths(data_name, suffix=""):
    base_dir = "./our_model_train_results/new_2.20_template_right/our_idea_weight"  # 基础目录
    if suffix:
...
data_name = "filtered_no_repeat_output_36788_0.40_1.00"  # 可以动态修改为其他名称
suffix = "epoch16_no_eval_our_idea_weight"  # 自定义后缀，可以根据需要修改，若不需要后缀则留空, no_eval _again
...
class DynamicWeightedSampler(Sampler):
    ...
    def __iter__(self):
        if self.current_epoch == 0:
            ...
            weights = torch.tensor(self.complexity_scores, dtype=torch.float32) + 1e-6
