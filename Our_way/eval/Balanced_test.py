...
# 加载微调后的模型
peft_model_id = "./our_model_train_results/new_2.20_template_right/our_idea_course_learning/epoch6/score_final_data_36788_gpt_correct_sig__no_eval_deepseek_change_again3/models"
# peft_model_id = "./our_model_train_results/new_2.20_template_right/original_way/final_all_MCQA_23880_data_new_score_all_test_epoch30_again2"
...
# 创建保存结果的文件夹
output_dir = "./our_test_results/test_sig_random1_test/score_final_data_36788_gpt_correct_sig__no_eval_deepseek_change_again3_course_learning_0.8113_again1"
os.makedirs(output_dir, exist_ok=True)

model = AutoModelForCausalLM.from_pretrained(peft_model_id, device_map="cuda", quantization_config=bnb_config, local_files_only=True)
...
# 打印当前的 padding_side 设置
print(f"Tokenizer padding side: {tokenizer.padding_side}")

# 加载测试数据集
test_data_path = "./our_test_results/all_test_data_sig_80_random1.json"
...
def predict_batch(texts, task_types, batch_size=16):
    ...
    for i in range(0, len(texts), batch_size):
        ...
        # 打印编码前的 padding_side 设置
        print(f"Encoding with padding side: {tokenizer.padding_side}")
